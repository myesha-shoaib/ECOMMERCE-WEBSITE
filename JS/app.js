...
function renderShop() { const q = (document.getElementById('q')?.value || '').toLowerCase(); const cat = document.getElementById('cat')?.value || 'all'; const sort = document.getElementById('sort')?.value || 'default'; const max = Number(document.getElementById('max')?.value || 50000); let list = products().filter(p => p.name.toLowerCase().includes(q) && (cat === 'all' || p.category === cat) && p.price <= max); if (sort === 'low') list.sort((a, b) => a.price - b.price); if (sort === 'high') list.sort((a, b) => b.price - a.price); if (sort === 'rating') list.sort((a, b) => b.rating - a.rating); document.getElementById('results').textContent = `${list.length} products`; document.getElementById('productGrid').innerHTML = list.length ? list.map(card).join('') : '<div class="card empty">No products found.</div>' }
...
function toggleWish(id) { let w = wishes(); w = w.includes(id) ? w.filter(x => x !== id) : [...w, id]; LS.set('wishlist', w); updateBadges(); if (document.getElementById('productGrid')) renderShop(); if (document.getElementById('featured')) renderFeatured(); if (document.getElementById('wishGrid')) renderWishlist() }
